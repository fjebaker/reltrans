...
contains
    ...
    type(krz_TraceResult) function trace_impact_parameters(mu_obs, alpha, beta,&
        ...
        ! The beta is mapped to negative, as kerrz has `beta` flipped from the
        ! convention. It is better to do that here than at the
        ! `trace_impact_parameters`
        ! callsite so that when it is changed in kerrz later it must only be
        ! updated in reltrans once.
