...
subroutine fbreltransWIp(ear, ne, param, ifl, photar)
  ...
  integer :: ne, ifl, Cp, dset
  real    :: ear(0:ne), param(23), photar(ne), par(32)
! Settings
  Cp   = 2   !|Cp|=2 means nthcomp, Cp>1 means there is a density parameter
  dset = 0   !dset=0 means distance is not set, logxi set instead
! Transfer to general parameter array
  ...
  par(7)  = param(8)         !zcos
  par(8)  = param(9)         !Gamma
  par(9)  = param(10)         !logxi
  par(10) = param(11)        !Afe
  par(11) = param(12)        !lognep
  par(12) = param(13)        !kTe
  par(13) = param(3)         !"phi_vel", i.e. velocity of the ring
  par(14) = 0.               ! not used
  par(15) = 0.               ! not used
  par(16) = param(14)        !Nh
  par(17) = param(15)        !boost
  par(18) = 1.0              !qboost
  par(19) = param(16)        !Mass
  par(20) = 0.0              ! not used
  par(21) = 0.0              ! not used
  par(22) = 0.0              ! not used
  par(23) = param(17)        !floHz
  par(24) = param(18)        !fhiHz
  par(25) = param(19)        !ReIm
  par(26) = param(20)        !DelA
  par(27) = param(21)        !DelAB
  par(28) = param(22)        !g
  par(29) = 0.               ! not used
  par(30) = 0.               ! not used
  par(31) = 1.0              !Anorm
  par(32) = param(23)        !telescope response
! Call general code
...
! Transfer to general parameter array
  ...
  par(32) = param(24)        !telescope response
...
! open file to write the lag simulation to
  ...
  ! read(*,'(a)')root
  root = 'test'
  lagfile =  'sim_' // trim(root) // '.dat'
  flxlagfile = 'sim_xspec_' // trim(root) // '.dat'
  phalagfile = 'sim_' // trim(root) // '.pha'
  rsplagfile = 'sim_' // trim(root) // '.rsp'
  status = 0
  call ftgiou(xunit,status)
  open(xunit,file=flxlagfile)
  call ftgiou(unit,status)
